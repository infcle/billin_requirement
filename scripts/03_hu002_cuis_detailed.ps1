# HU-002 - Renovar CUIS Automaticamente (Detailed Sub-tasks)
# Usage: .\03_hu002_cuis_detailed.ps1 [-WhatIf]
...
Write-Host "=== HU-002: Renovar CUIS Automaticamente (Detailed) ===" -ForegroundColor Cyan
...
$apiTasks = @(
    @{ id = "1"; title = "Crear entity CUIS y repository"; desc = "Crear JPA entity CUIS con campos: id, codigoCuis, fechaEmision, fechaVencimiento, activo. Crear CUISRepository."; est = 1 },
    @{ id = "2"; title = "Implementar servicio de renovacion CUIS"; desc = "Crear CUISService con metodos: renovarCUIS(), verificarExpiracion(), obtenerCUISActivo()."; est = 1 },
    @{ id = "3"; title = "Implementar job programado (cron/scheduler)"; desc = "Configurar @Scheduled para ejecutar renovacion. Verificar CUIS que expiran en 30 dias."; est = 1 },
    @{ id = "4"; title = "Integrar con servicio SIN para renovacion"; desc = "Crear SINClient con metodo renovarCUIS(). Manejar respuestas exitosas y errores."; est = 1 },
    ...
    @{ id = "6"; title = "Tests unitarios"; desc = "Crear unit tests para CUISService, SINClient y job programado."; est = 1 },
...
foreach ($task in $apiTasks) {
    $taskTitle = "[API]HU-002-$($task.id) $($task.title)"
...
$feTasks = @(
    @{ id = "1"; title = "Dashboard de estado CUIS"; desc = "Crear componente CUISDashboardComponent. Mostrar CUIS actual, fecha vencimiento, estado."; est = 1 },
    @{ id = "2"; title = "Indicador de proxima renovacion"; desc = "Implementar alerta visual cuando CUIS expire en 30 dias. Badge con countdown."; est = 1 },
    @{ id = "3"; title = "Boton de renovacion manual"; desc = "Crear boton para renovar CUIS manualmente. Confirmacion y loading state."; est = 1 },
...
foreach ($task in $feTasks) {
    $taskTitle = "[FE]HU-002-$($task.id) $($task.title)"
    $taskDesc = "$($task.desc)`n`nParent: $($parent.identifier)"
...
$qaTasks = @(
    @{ id = "1"; title = "Probar renovacion automatica 30 dias antes"; desc = "Simular CUIS que expira en 30 dias. Verificar que se renueva automaticamente."; est = 1 },
    @{ id = "2"; title = "Simular fallo de servicio SIN"; desc = "Mock SIN para retornar error 500. Verificar retry logic y backoff."; est = 1 },
    ...
    @{ id = "4"; title = "Validar job programado"; desc = "Verificar que job se ejecuta segun cron. Testear con diferentes horarios."; est = 1 },
    @{ id = "5"; title = "Tests de carga"; desc = "Probar renovacion simultanea de multiples CUIS. Verificar performance."; est = 1 }
)

foreach ($task in $qaTasks) {
    $taskTitle = "[QA]HU-002-$($task.id) $($task.title)"
    $taskDesc = "$($task.desc)`n`nParent: $($parent.identifier)"
...
Write-Host "Next: Run 04_hu003_cufd_detailed.ps1" -ForegroundColor Cyan
