...
# Usage: .\06_hu014_retry.ps1 [-WhatIf]
...
Write-Host "=== HU-014: Retry Logic y Resiliencia ===" -ForegroundColor Cyan
...
$apiTitle = "[API]HU-014 Implementar Retry Logic y Resiliencia"
$apiDesc = "Backend implementation tasks:`n- Implementar RetryTemplate global con backoff exponencial`n- Mapeo de errores HTTP recuperables (500/502/503)`n- Configuracion de maximos reintentos por endpoint`n- Circuit breaker pattern`n- Logging de reintentos`n- Tests de resiliencia`n- Configuracion de timeouts`n`nParent: $($parent.identifier)"
...
$feTitle = "[FE]HU-014 Implementar Retry Logic y Resiliencia"
$feDesc = "Frontend implementation tasks:`n- Indicador de reintentos en UI`n- Toast de error con boton de retry manual`n- Estado de loading con contador de reintentos`n- Deshabilitar acciones durante retry`n- Historial de reintentos`n- Tests E2E`n`nParent: $($parent.identifier)"
...
$qaTitle = "[QA]HU-014 Implementar Retry Logic y Resiliencia"
$qaDesc = "QA testing tasks:`n- Simular errores 500/502/503`n- Verificar backoff exponencial`n- Probar circuit breaker`n- Validar no-duplicacion de requests`n- Tests de carga con fallos`n- Verificar retry manual`n`nParent: $($parent.identifier)"
...
Write-Host "Next: Run 07_hu013_seguridad.ps1" -ForegroundColor Cyan
