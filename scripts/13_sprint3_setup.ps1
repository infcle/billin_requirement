# Sprint 3 Setup - Create Sprint 3 Cycle
# Usage: .\13_sprint3_setup.ps1 [-WhatIf]
...
Write-Host "=== Sprint 3 Setup ===" -ForegroundColor Cyan
...
# Create/Get Sprint 3 Cycle
Write-Host "`nSprint 3 Cycle..." -ForegroundColor Cyan
$cycleName = "Sprint 3: Contingencias y Paquetes"
...
if ($cycleR.ok -and $cycleR.data.cycles.nodes.Count -gt 0) {
    ...
    $cycleM = 'mutation { cycleCreate(input: { name: "' + $cycleName + '", startsAt: "2026-05-21T00:00:00Z", endsAt: "2026-06-17T23:59:59Z", teamId: "' + $setup.teamId + '" }) { success cycle { id name } } }'
...
$sprint3Data | ConvertTo-Json | Set-Content "D:\Facturacion en linea\scripts\sprint3_setup.json"

Write-Host "`n=== Sprint 3 Setup Complete ===" -ForegroundColor Green
Write-Host "Project: $($setup.projectName)" -ForegroundColor White
Write-Host "Cycle: $($cycle.name)" -ForegroundColor White
Write-Host "Setup saved to sprint3_setup.json" -ForegroundColor Gray
Write-Host "`nNext: Run 14_hu007_contingencias_detailed.ps1" -ForegroundColor Cyan
